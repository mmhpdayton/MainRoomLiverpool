...
ESPN_SCOREBOARD='https://site.api.espn.com/apis/site/v2/sports/soccer/{league}/scoreboard?limit=1000&dates=2026'
...
def espn_cup(league):
    payload=fetch_json(ESPN_SCOREBOARD.format(league=league));out=[]
    for e in payload.get('events') or []:
        comp=(e.get('competitions') or [{}])[0];teams=comp.get('competitors') or []
        if len(teams)<2:continue
        home=next((x for x in teams if x.get('homeAway')=='home'),teams[0]);away=next((x for x in teams if x.get('homeAway')=='away'),teams[1])
        status=(e.get('status') or {}).get('type') or {};state=status.get('state') or '';completed=bool(status.get('completed'))
        def score(x):
            try:return int(x.get('score')) if x.get('score') not in (None,'') else None
            except:return None
        notes=comp.get('notes') or [];round_text=' '.join(str(n.get('headline') or '') for n in notes).strip()
        rnd=0
        for token in round_text.replace('-',' ').split():
            if token.isdigit():rnd=int(token);break
        venue=((comp.get('venue') or {}).get('fullName') or '')
        out.append({'id':str(e.get('id') or ''),'round':rnd,'roundName':round_text,'date':e.get('date') or '',
                    'home':norm_team((home.get('team') or {}).get('displayName')),'away':norm_team((away.get('team') or {}).get('displayName')),
                    'homeScore':score(home),'awayScore':score(away),'status':'final' if completed else ('live' if state=='in' else 'scheduled'),
                    'liveStatus':status.get('shortDetail') or status.get('detail') or '', 'venue':venue,'source':'ESPN competition scoreboard'})
    if not out:raise RuntimeError(f'{league} ESPN feed returned no matches')
    return sorted(out,key=lambda x:x.get('date',''))

def find_match(comp,home,away,league=''):
    league=(league or '').lower()
    if 'champion' in league:key='championsLeagueMatches'
    elif 'premier' in league:key='premierLeagueMatches'
    elif 'fa cup' in league:key='faCupMatches'
    elif 'league cup' in league or 'carabao' in league:key='milkCupMatches'
    else:key=None
    buckets=[comp.get(key,[])] if key else [comp.get('championsLeagueMatches',[]),comp.get('premierLeagueMatches',[]),comp.get('faCupMatches',[]),comp.get('milkCupMatches',[])]
...
def apply_sportsdb_event(comp,e,is_live=False):
    ...
    if not any(x in ll for x in ('champion','premier','fa cup','league cup','carabao')):return False
    ...
    if is_live:status='final' if 'final' in pl or 'finished' in pl else 'live'
    else:status='final' if ('final' in pl or 'finished' in pl or (hs is not None and as_ is not None and not progress)) else g.get('status','scheduled')
...
def ucl_table(matches):
    teams=sorted({g['home'] for g in matches}|{g['away'] for g in matches});stats={t:{'team':t,'p':0,'w':0,'d':0,'l':0,'gf':0,'ga':0,'gd':0,'pts':0} for t in teams}
    for g in matches:
...
def sync_liverpool(site,comp):
    ...
    for competition,key in [('Premier League','premierLeagueMatches'),('Champions League','championsLeagueMatches')]:
        for m in comp.get(key,[]):
            ...
            ha='H' if m['home']=='Liverpool' else 'A';opp=m['away'] if ha=='H' else m['home'];k=(competition,opp,ha);g=index.get(k)
            if not g:
            ...
            if m.get('homeScore') is not None:g['scoreFor']=m['homeScore'] if ha=='H' else m['awayScore'];g['scoreAgainst']=m['awayScore'] if ha=='H' else m['homeScore']
            g['liveStatus']=m.get('liveStatus','')
    site['fixtures']=sorted(fixtures,key=lambda x:x.get('date',''));site['championsLeagueTable']=ucl_table(comp.get('championsLeagueMatches',[]));site['championsLeagueStatus']='2026–27 league phase • 36 teams • 8 matchdays'

def main():
    live_only='--live' in sys.argv
    comp=load(OUT,{'premierLeagueMatches':[],'championsLeagueMatches':[],'faCupMatches':[],'milkCupMatches':[]});site=load(SITE,{})
    ...
    if not live_only:
        ...
        try:comp['milkCupMatches']=espn_cup('eng.league_cup');health['milkCupMatches']=f"full competition scoreboard loaded ({len(comp['milkCupMatches'])} matches)"
        except Exception as e:print('Milk Cup competition feed',e);health['milkCupMatches']='preserved last-known-good schedule'
        try:comp['faCupMatches']=espn_cup('eng.fa');health['faCupMatches']=f"full competition scoreboard loaded ({len(comp['faCupMatches'])} matches)"
        except Exception as e:print('FA Cup competition feed',e);health['faCupMatches']='preserved last-known-good schedule'
